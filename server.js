...
const testEmailConnection = async () => {
    if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
        ...
        if (connected) {
            ...
            console.warn('⚠️ Email service configuration issue. Emails may not be sent.');
        }
    } else {
        console.warn('⚠️ Email credentials not found in .env file');
        console.warn('   Email notifications will not be sent');
        console.warn('   Add EMAIL_USER and EMAIL_PASSWORD to enable email notifications');
    }
};
...
// Approve registration (admin only) - WITH EMAIL NOTIFICATION
app.put('/api/admin/registration/:id/approve', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { approvedBy = req.user.username || 'Admin' } = req.body;
        ...
        console.log(`✅ Registration ${id} approved by ${approvedBy}`);
        console.log(`👤 User: ${registration.fullName} (${registration.email})`);
        ...
        if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
            try {
                ...
                console.log('⚠️ Registration approved but email notification failed');
                // Don't fail the request if email fails
            }
        } else {
            console.log('⚠️ Email credentials not configured - approval email not sent');
        }
...
// Reject registration (admin only) - WITH REJECTION EMAIL
app.put('/api/admin/registration/:id/reject', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { reason = '' } = req.body;
        ...
        if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
            try {
                await EmailService.sendRejectionEmail(registration, reason.trim());
                console.log(`📧 Rejection email sent to: ${registration.email}`);
            } catch (emailError) {
                console.error('❌ Failed to send rejection email:', emailError.message);
                console.log('⚠️ Registration rejected but email notification failed');
                // Don't fail the request if email fails
            }
        } else {
            console.log('⚠️ Email credentials not configured - rejection email not sent');
        }
...
app.listen(PORT, () => {
    ...
    console.log(`📧 Email check: POST /api/check-email`);
